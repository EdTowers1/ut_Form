...
Function Conect_mysql(adatos)
	...
	TRY
		oServer = TDolphinSrv():New(hIni[ c ]["host"]       ,;
			hIni[ c ]["user"]       ,;
			hIni[ c ]["psw"]        ,;
			val(hIni[ c ]["port"])  ,;
			val(hIni[ c ]["flags"]) ,;
	...
	/*
   IF ! oServer:lError
      oQry:= oServer:Query( "SELECT count(*) as cuantos FROM ma_cuentas" )
      adatos[1]:=oQry:cuantos

      oQry:= oServer:Query( "SELECT * FROM ma_clientes WHERE Row_id=1" )
	  adatos[2]:=oQry:FillHRow()
...
Function getClientes()
	...
	IF ! oConnection:lError
		oQry := oConnection:Query( "SELECT row_id, codcli, nomcli FROM munmacli LIMIT 25;")
      
			
		IF oQry != NIL
			oQry:gotop()
			DO WHILE ! oQry:Eof()
				aRow := { 'CODE' => oQry:codcli, 'FULLNAME' => oQry:nomcli }
				AADD( aClientes, aRow )
				oQry:Skip()
			END
		ENDIF
	...
	// Retornar datos en formato JSON directamente como array
	// UWrite( HB_JSONENCODE( aClientes ) )
